...
<?php
    ...
    $userId = $_GET["id"];
    $myId = $_GET["myid"];
    $alreadyfriend = $_GET["friend"];
    if(isset($_GET["makefriend"]))
        $makeFriend = $_GET["makefriend"];
    ...
    $results = $conn->query($query);
    if($results->num_rows > 0){
            while($userInfo = $results->fetch_assoc()){
                $name = $userInfo['firstname']." ".$userInfo['lastname'];
                $email = $userInfo['email'];
                $profilePicAddr = $userInfo['profilePic'];
                break;
            }
        }
    $tableName = "UserDesc";
    ...
    $results = $conn->query($query);
    if($results->num_rows > 0){
            while($userOtherInfo = $results->fetch_assoc()){
                $city = $userOtherInfo['city'];
                $gender = $userOtherInfo['gender'];
                $quote = $userOtherInfo['quote'];
                $dob = $userOtherInfo['dob'];
                break;
            }
        }

        if(isset($makeFriend) && $makeFriend==="yes"){
            $servername = "localhost";
            $username = "root";
            $passwordDB = "hamunaptra";
            $dbName = "chatUserDB";
            $tableName = "FriendReq";
            $query1 = "SELECT * from ".$tableName." WHERE userID=".$userId;
            $conn = new mysqli($servername, $username, $passwordDB, $dbName);
            if ($conn->connect_error) {
                die("Connection failed: " . $conn->connect_error);
            } 
            ...
            if($flag === TRUE){
                echo "HERE";
                $query = "INSERT INTO ".$tableName." (userID, reqUserId) 
                VALUES (".$userId.", ".$myId.");";
                $conn->query($query);
            }
        }
...
<head>
    ...
    <style type="text/css">
        body {
            background-image: url("http://www.planwallpaper.com/static/images/christmas-background.jpg");
        }
        #panelSection {
...
<h2>User Info for <?php echo $name;?></h2>
<?php 
if($alreadyfriend=="no"){
echo '<div id="welcome" class="panel panel-info"><div class="panel-heading">Do you want to be friends with '.$name.'?</div></div>';
}else{
    echo '<div id="welcome" class="panel panel-info"><div class="panel-heading">You are friends with  '.$name.'!</div></div>';
}
?>
<div class="panel panel-default" id="panelSection">
    <img src="<?php echo $profilePicAddr;?>" id="profilePic1" class="img-circle" alt="profilePic" width="150" height="150">
    <div class="container" id="infoSection">
        <p class="bg-info">E-mail : <?php echo $email;?></p>
        <p class="bg-info">Name : <?php echo $name;?></p>
        <p class="bg-info">City : <?php echo $city;?></p>
        <p class="bg-info">DOB : <?php echo $dob;?></p>
        <p class="bg-info">Quote : <?php echo $quote;?></p>
        <p class="bg-info">Gender : <?php echo $gender;?></p>
    </div>
</div>

<?php
if($alreadyfriend=="no"){
    echo '<form class="form" action= "web6.php?friend=no&username='.$myUsername.'&myid='.$myId.'&id='.$userId.'&makefriend=yes" method="post"><div class="form-group"><button type="submit" class="btn btn-primary btn-block">I want to be friends!</button></div></form>';
}
?>
<footer>
